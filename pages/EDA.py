...
#df.info()
#df.describe()
st.header("EDA ANALYSIS OF OUR DATASET")
...
st.markdown("### 1.Average Monthly solar production from all the sites")
df_production.groupby(df_production.index.month).mean().plot(kind="bar",xlabel='Month',ylabel='energy in KWh',title="Average Monthly solar production from all the sites", figsize=(20,12))
st.pyplot(plt.gcf())
st.write("The average production of solar power over different months in years 2015 - 2020 is shown here. Comparitively Germany produce the maximum amount of electricity. The Hungary produce the lowest amount of power.")
...
df_production_2020.groupby(df_production_2020.index.year).sum().plot(kind="bar",xlabel='Year',ylabel='energy in Watt',title="Total production in 2020 By different regions", figsize=(16,6))
st.pyplot(plt.gcf())
...
plt.show()
st.pyplot(plt)
st.write("This plot displays the production of both solar and wind energy in Austria over 6 years. Austria is getting have more wind energy production than solar.")